...
export async function getAllServices(req, res) {
  try {
    ...
    // Attach images to services and calculate discounted price (only for fixed price_type)
    const servicesWithImages = rows.map(service => {
      ...
      // Only calculate discount for fixed price services
      ...
      return {
        ...service,
        price: originalPrice,
        original_price: (service.price_type === 'fixed' && discountPercentage > 0) ? originalPrice : null,
        discounted_price: (service.price_type === 'fixed' && discountPercentage > 0) 
    ...
    try {
      ...
      const [result] = await db.query(
        `INSERT INTO services (
          ...
          specifications, service_type, price_type, price, discount_percentage, warranty_info, 
      ...
      const [rows] = await db.query(
        `SELECT 
          ...
          specifications, service_type, price_type, price, discount_percentage, average_rating, review_count, 
